...
function Recipes({recipes,onNew,onOpen}:any){return <><div className="toolbar"><div><span className="eyebrow">ARCHIVIO CUCINA</span><h2 className="sectionTitle">Ricettario</h2></div><button className="secondary" onClick={onNew}><Plus size={17}/> Nuova ricetta</button></div><div className="recipeCards">{recipes.map((r:Recipe)=>{const total=recipeCost(r);const suggested=suggestedPrice(r);return <button className="recipeCard" key={r.id} onClick={()=>onOpen(r)}><div className="recipeCardTop"><div className="iconDisc"><ChefHat/></div><div><span className="eyebrow">{r.category||'Senza categoria'}</span><h3>{r.name}</h3><p>{(r.recipe_ingredients||[]).length} ingredienti</p></div><Pencil size={17}/></div><div className="recipeMetrics"><div><span>Costo ricetta</span><strong>{euro(total)}</strong></div><div><span>Prezzo consigliato</span><strong className="accentText">{suggested?euro(suggested):'—'}</strong></div></div></button>})}</div>{recipes.length===0&&<Empty text="Il ricettario è vuoto. Creiamo la prima ricetta."/>}</>}
...
function RecipeModal({recipe,ingredients,units,onClose,onSaved}:{recipe:Recipe|null;ingredients:Ingredient[];units:Unit[];onClose:()=>void;onSaved:()=>Promise<void>}){
  const [f,setF]=useState({name:recipe?.name||'',category:recipe?.category||'',target_food_cost_percent:recipe?.target_food_cost_percent?.toString()||'30',manual_sale_price:recipe?.manual_sale_price?.toString()||'',procedure:recipe?.procedure||'',notes:recipe?.notes||''})
