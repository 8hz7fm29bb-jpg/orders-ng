...
function costPerPortion(recipe: Recipe) {
  return recipeCost(recipe) / Math.max(Number(recipe.standard_portions || 1), 0.0001)
}

function suggestedPrice(recipe: Recipe) {
  const target = Number(recipe.target_food_cost_percent || 0)
  if (!target) return 0
  return costPerPortion(recipe) / (target / 100)
}
...
function Recipes({recipes,onNew,onOpen}:any){return <><div className="toolbar"><div><span className="eyebrow">ARCHIVIO CUCINA</span><h2 className="sectionTitle">Ricettario</h2></div><button className="secondary" onClick={onNew}><Plus size={17}/> Nuova ricetta</button></div><div className="recipeCards">{recipes.map((r:Recipe)=>{const total=recipeCost(r);const per=costPerPortion(r);const suggested=suggestedPrice(r);return <button className="recipeCard" key={r.id} onClick={()=>onOpen(r)}><div className="recipeCardTop"><div className="iconDisc"><ChefHat/></div><div><span className="eyebrow">{r.category||'Senza categoria'}</span><h3>{r.name}</h3><p>{(r.recipe_ingredients||[]).length} ingredienti · {r.standard_portions} porzioni</p></div><Pencil size={17}/></div><div className="recipeMetrics"><div><span>Costo ricetta</span><strong>{euro(total)}</strong></div><div><span>Costo / porzione</span><strong>{euro(per)}</strong></div><div><span>Prezzo consigliato</span><strong className="accentText">{suggested?euro(suggested):'—'}</strong></div></div></button>})}</div>{recipes.length===0&&<Empty text="Il ricettario è vuoto. Creiamo la prima ricetta."/>}</>}
...
function RecipeModal({recipe,ingredients,units,onClose,onSaved}:{recipe:Recipe|null;ingredients:Ingredient[];units:Unit[];onClose:()=>void;onSaved:()=>Promise<void>}){
  const [f,setF]=useState({name:recipe?.name||'',category:recipe?.category||'',standard_portions:String(recipe?.standard_portions||1),target_food_cost_percent:recipe?.target_food_cost_percent?.toString()||'30',manual_sale_price:recipe?.manual_sale_price?.toString()||'',procedure:recipe?.procedure||'',notes:recipe?.notes||''})
  ...
  const per=total/Math.max(Number(f.standard_portions||1),0.0001)
  const suggested=Number(f.target_food_cost_percent||0)>0?per/(Number(f.target_food_cost_percent)/100):0
  ...
  async function save(e:FormEvent){e.preventDefault();if(!supabase)return;if(lines.some(l=>!l.ingredient_id||!l.unit_id||Number(l.quantity)<=0))return alert('Completa ingrediente, quantità e unità di misura per ogni riga.');setSaving(true);const payload={name:f.name,category:f.category||null,standard_portions:Number(f.standard_portions),target_food_cost_percent:f.target_food_cost_percent?Number(f.target_food_cost_percent):null,manual_sale_price:f.manual_sale_price?Number(f.manual_sale_price):null,procedure:f.procedure||null,notes:f.notes||null};let recipeId=recipe?.id||'';if(recipe){const {error}=await supabase.from('recipes').update(payload).eq('id',recipe.id);if(error){setSaving(false);return alert(error.message)}}else{const {data,error}=await supabase.from('recipes').insert(payload).select('id').single();if(error){setSaving(false);return alert(error.message)}recipeId=(data as any).id}const del=await supabase.from('recipe_ingredients').delete().eq('recipe_id',recipeId);if(del.error){setSaving(false);return alert(del.error.message)}if(lines.length){const rows=lines.map((l,idx)=>({recipe_id:recipeId,ingredient_id:l.ingredient_id,quantity:Number(l.quantity),unit_id:l.unit_id,sort_order:idx}));const ins=await supabase.from('recipe_ingredients').insert(rows);if(ins.error){setSaving(false);return alert(ins.error.message)}}setSaving(false);await onSaved();onClose()}
  return <Modal title={recipe?`Modifica ricetta — ${recipe.name}`:'Nuova ricetta'} onClose={onClose} wide><form onSubmit={save} className="recipeEditor"><div className="formGrid recipeBasics"><label className="span2">Nome piatto<input required value={f.name} onChange={e=>setF({...f,name:e.target.value})}/></label><label>Categoria<input value={f.category} onChange={e=>setF({...f,category:e.target.value})} placeholder="Antipasto, Primo…"/></label><label>Porzioni standard<input type="number" min="0.01" step="0.01" value={f.standard_portions} onChange={e=>setF({...f,standard_portions:e.target.value})}/></label><label>Food cost target %<input type="number" min="1" max="100" step="0.1" value={f.target_food_cost_percent} onChange={e=>setF({...f,target_food_cost_percent:e.target.value})}/></label><label>Prezzo vendita manuale €<input type="number" min="0" step="0.01" value={f.manual_sale_price} onChange={e=>setF({...f,manual_sale_price:e.target.value})}/></label></div><div className="recipeWork"><section className="compositionBox"><div className="compositionHead"><div><span className="eyebrow">RICETTA E COMPOSIZIONE</span><h3>Ingredienti necessari</h3></div><button type="button" className="secondary" onClick={addLine}><Plus size={16}/> Ingrediente</button></div>{lines.length===0?<Empty text="Aggiungi gli ingredienti della ricetta."/>:<div className="ingredientEditorList"><div className="ingredientEditorHeader"><span>Ingrediente</span><span>Quantità</span><span>Unità</span><span>Costo unit.</span><span>Subtotale</span><span></span></div>{lines.map((l,idx)=>{const ing=ingredients.find(i=>i.id===l.ingredient_id);const hyd=hydratedLines[idx];return <div className="ingredientEditorRow" key={idx}><select value={l.ingredient_id} onChange={e=>updateLine(idx,{ingredient_id:e.target.value})}><option value="">Seleziona ingrediente</option>{ingredients.filter(i=>i.active).map(i=><option key={i.id} value={i.id}>{i.name}</option>)}</select><input type="number" min="0" step="0.0001" value={l.quantity} onChange={e=>updateLine(idx,{quantity:e.target.value})} placeholder="0"/><select value={l.unit_id} onChange={e=>updateLine(idx,{unit_id:e.target.value})}><option value="">Unità</option>{units.map(u=><option key={u.id} value={u.id}>{u.code}</option>)}</select><span>{ing?`${euro(ing.current_price)} / ${ing.unit?.code||''}`:'—'}</span><strong>{euro(lineCost(hyd))}</strong><button type="button" className="deleteLine" onClick={()=>removeLine(idx)}><Trash2 size={16}/></button></div>})}<div className="recipeTotal"><span>Totale materie prime</span><strong>{euro(total)}</strong></div></div>}</section><aside className="costPanel"><span>PREZZO DI VENDITA CONSIGLIATO</span><strong>{suggested?euro(suggested):'—'}</strong><div className="costRows"><div><span>Costo food reale</span><b>{euro(total)}</b></div><div><span>Costo per porzione</span><b>{euro(per)}</b></div><div><span>Food cost target</span><b>{f.target_food_cost_percent?`${f.target_food_cost_percent}%`:'—'}</b></div></div></aside></div><div className="formGrid recipeNotes"><label className="span2">Note e procedimento<textarea rows={5} value={f.procedure} onChange={e=>setF({...f,procedure:e.target.value})} placeholder="Procedimento di preparazione…"/></label><label className="span2">Note interne<textarea rows={3} value={f.notes} onChange={e=>setF({...f,notes:e.target.value})}/></label></div><div className="formActions recipeActions"><button type="button" className="ghost" onClick={onClose}>Annulla</button><button className="primary" disabled={saving}>{saving?'Salvataggio…':recipe?'Salva modifiche':'Salva ricetta'}</button></div></form></Modal>
}
