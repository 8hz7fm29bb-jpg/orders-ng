...
function RecipeModal({recipe,ingredients,units,onClose,onSaved}:{recipe:Recipe|null;ingredients:Ingredient[];units:Unit[];onClose:()=>void;onSaved:()=>Promise<void>}){
  ...
  return <Modal title={recipe?`Modifica ricetta — ${recipe.name}`:'Nuova ricetta'} onClose={onClose} wide><form onSubmit={save} className="recipeEditor"><div className="formGrid recipeBasics"><label className="span2">Nome piatto<input required value={f.name} onChange={e=>setF({...f,name:e.target.value})}/></label><label>Categoria<select required value={f.category} onChange={e=>setF({...f,category:e.target.value})}><option value="">Seleziona categoria</option>{RECIPE_CATEGORIES.map(category=><option key={category} value={category}>{category}</option>)}</select></label><label>Food cost target %<input type="number" min="1" max="100" step="0.1" value={f.target_food_cost_percent} onChange={e=>setF({...f,target_food_cost_percent:e.target.value})}/></label><label>Prezzo vendita manuale €<input type="number" min="0" step="0.01" value={f.manual_sale_price} onChange={e=>setF({...f,manual_sale_price:e.target.value})}/></label></div><div className="recipeWork"><section className="compositionBox"><div className="compositionHead"><div><span className="eyebrow">RICETTA E COMPOSIZIONE</span><h3>Ingredienti necessari</h3></div><button type="button" className="secondary" onClick={addLine}><Plus size={16}/> Ingrediente</button></div>{lines.length===0?<Empty text="Aggiungi gli ingredienti della ricetta."/>:<div className="ingredientEditorList"><div className="ingredientEditorHeader"><span>Ingrediente</span><span>Quantità</span><span>Unità</span><span>Costo unit.</span><span>Subtotale</span><span></span></div>{lines.map((l,idx)=>{const ing=ingredients.find(i=>i.id===l.ingredient_id);const hyd=hydratedLines[idx];return <div className="ingredientEditorRow" key={idx}><select value={l.ingredient_id} onChange={e=>updateLine(idx,{ingredient_id:e.target.value})}><option value="">Seleziona ingrediente</option>{ingredients.filter(i=>i.active).map(i=><option key={i.id} value={i.id}>{i.name}</option>)}</select><input type="number" min="0" step="0.0001" value={l.quantity} onChange={e=>updateLine(idx,{quantity:e.target.value})} placeholder="0"/><select value={l.unit_id} onChange={e=>updateLine(idx,{unit_id:e.target.value})}><option value="">Unità</option>{units.map(u=><option key={u.id} value={u.id}>{u.code}</option>)}</select><span>{ing?`${euro(ing.current_price)} / ${ing.unit?.code||''}`:'—'}</span><strong>{euro(lineCost(hyd))}</strong><button type="button" className="deleteLine" onClick={()=>removeLine(idx)}><Trash2 size={16}/></button></div>})}<div className="recipeTotal"><span>Totale materie prime</span><strong>{euro(total)}</strong></div></div>}</section><aside className="costPanel"><span>PREZZO DI VENDITA CONSIGLIATO</span><strong>{suggested?euro(suggested):'—'}</strong><div className="costRows"><div><span>Costo food reale</span><b>{euro(total)}</b></div><div><span>Food cost target</span><b>{f.target_food_cost_percent?`${f.target_food_cost_percent}%`:'—'}</b></div></div></aside></div><div className="formGrid recipeNotes"><label className="span2">Note e procedimento<textarea rows={5} value={f.procedure} onChange={e=>setF({...f,procedure:e.target.value})} placeholder="Procedimento di preparazione…"/></label><label className="span2">Note interne<textarea rows={3} value={f.notes} onChange={e=>setF({...f,notes:e.target.value})}/></label></div><div className="formActions recipeActions"><button type="button" className="ghost" onClick={onClose}>Annulla</button><button className="primary" disabled={saving}>{saving?'Salvataggio…':recipe?'Salva modifiche':'Salva ricetta'}</button></div></form></Modal>
}
